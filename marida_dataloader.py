    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            stack = np.concatenate([img, target], axis=-1).astype(
                'float32')  # In order to rotate-transform both mask and image
...
if __name__ == "__main__":
    path = '/home/kno/datasets/MARIDA/'
    train_patches = np.genfromtxt(os.path.join(path, 'splits', 'train_X.txt'), dtype='str')
    val_patches = np.genfromtxt(os.path.join(path, 'splits', 'val_X.txt'), dtype='str')
    test_patches = np.genfromtxt(os.path.join(path, 'splits', 'test_X.txt'), dtype='str')

    all_cl = np.zeros(16)
    all_conf = np.zeros(4)
    for roi in train_patches:
        # Construct file and folder name from roi
        roi_folder = '_'.join(['S2'] + roi.split('_')[:-1])  # Get Folder Name
        roi_name = '_'.join(['S2'] + roi.split('_'))  # Get File Name
        # roi_file = os.path.join(path, 'patches', roi_folder, roi_name + '.tif')  # Get File path
        roi_file_cl = imageio.imread(os.path.join(path, 'patches', roi_folder, roi_name + '_cl.tif'))
        roi_file_conf = imageio.imread(os.path.join(path, 'patches', roi_folder, roi_name + '_conf.tif'))
        all_cl += np.bincount(roi_file_cl.flatten().astype(int), minlength=16)
        all_conf += np.bincount(roi_file_conf.flatten().astype(int), minlength=4)
